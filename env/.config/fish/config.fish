...
# Platform-specific paths - nvim detection
if test "$OS_TYPE" = "Linux"
    # Try multiple nvim locations (ordered by preference)
    for nvim_path in /opt/nvim-linux-x86_64/bin /usr/local/bin /usr/bin
        if test -x $nvim_path/nvim
            set -gx PATH $nvim_path $PATH
            break
        end
    end
    
    # CMake - check if executable exists before adding to PATH
    if test -x $HOME/Downloads/cmake-3.31.0-rc2-linux-x86_64/bin/cmake
        set -gx PATH $HOME/Downloads/cmake-3.31.0-rc2-linux-x86_64/bin $PATH
    end
end
...
if test "$OS_TYPE" = "Darwin"
    ...
    if set -q HOMEBREW_PREFIX
        ...
        # macOS nvim from Homebrew
        if test -x $HOMEBREW_PREFIX/bin/nvim
            # Already in PATH from above
        end
    end
    
    # macOS CMake - check if executable exists
    if test -x $HOME/Downloads/cmake-3.31.0-rc2-macos-universal/bin/cmake
        set -gx PATH $HOME/Downloads/cmake-3.31.0-rc2-macos-universal/bin $PATH
    end
end
...
# LS_COLORS - using system defaults
